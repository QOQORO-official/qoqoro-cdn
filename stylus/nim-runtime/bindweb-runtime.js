/**
 * Nim Bindweb Browser Runtime -- vendored for the stylus note app.
 * Generated from schema.def by the BindWeb IDE, then loaded as a classic
 * script. Local changes are marked "stylus:"; the stylus-specific imports
 * themselves live in stylus-host.js.
 */

function createBindwebRunner(outputContainer) {
    ...
    // stylus: pointer samples, pictures, downloads and dialogs come from
    // stylus-host.js; it reads memory through a getter because memory is
    // only assigned on connect.
    const stylusImports = (typeof createStylusHost === 'function') ? createStylusHost({
        get memory() { return memory; },
        elements, contexts, images, allocHandle,
        trigger: () => _triggerDiscreteUpdate(),
    }) : {};
    const envImports = {
        ...stylusImports,
...
,
        bindweb_fetch_get: (url_ptr, url_len, headers_ptr, headers_len) => {
            ...
            const id = allocHandle({ fetch: true }); let h = {}; try { h = JSON.parse(headers); } catch(e) {} fetch(url, { headers: h }).then(r => r.text().then(d => ({ ok: r.ok, status: r.status, statusText: r.statusText, data: d }))).then(res => { if(res.ok) push_event_fetch_SUCCESS(id, res.data); else push_event_fetch_ERROR(id, res.data && res.data.length ? res.data : (res.status + ' ' + res.statusText)); }).catch(e => push_event_fetch_ERROR(id, e.toString())); return id;
        }
,
...
,
        bindweb_fetch_patch: (url_ptr, url_len, body_ptr, body_len, headers_ptr, headers_len) => {
            ...
            const body = decoder.decode(new Uint8Array(memory.buffer, body_ptr, body_len));
            const headers = decoder.decode(new Uint8Array(memory.buffer, headers_ptr, headers_len));
            const id = allocHandle({ fetch: true }); let h = {}; try { h = JSON.parse(headers); } catch(e) {} fetch(url, { method: 'PATCH', body: body, headers: h }).then(r => r.text().then(d => ({ ok: r.ok, status: r.status, statusText: r.statusText, data: d }))).then(res => { if(res.ok) push_event_fetch_SUCCESS(id, res.data); else push_event_fetch_ERROR(id, res.data && res.data.length ? res.data : (res.status + ' ' + res.statusText)); }).catch(e => push_event_fetch_ERROR(id, e.toString())); return id;
        }
,
        ...
        while (pos < end) {
            ...
            switch (opcode) {
                ...
                case 13: {
                    ...
                    if (pos + value_pad > end) break;
                    const value = decoder.decode(u8.subarray(pos, pos + value_len)); pos += value_pad;
                    const el = elements[handle]; if(!el){ console.warn('set_property: unknown element handle', handle); continue; } el[name] = (name === 'checked' || name === 'disabled') ? value === 'true' : value; // stylus: boolean properties
                    break;
                }
...
// stylus: classic script, no default export
