        ...
        while (pos < end) {
            ...
            switch (opcode) {
                ...
                case 13: {
                    ...
                    if (pos + value_pad > end) break;
                    const value = decoder.decode(u8.subarray(pos, pos + value_len)); pos += value_pad;
                    const el = elements[handle]; if(!el){ console.warn('set_property: unknown element handle', handle); continue; }
                    // recto: boolean DOM properties arrive as strings from Nim.
                    el[name] = (name === 'checked' || name === 'disabled') ? (value === 'true' || value === '1') : value;
                    break;
                }
